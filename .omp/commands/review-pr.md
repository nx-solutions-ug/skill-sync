...
- If `IS_JULES=true` → **Jules-involved PR**: follow Step 5d
...
Dependency PRs do NOT use the inline-review submission in Step 8. Stop after posting the summary comment.
...
4. Submit the review per Step 8 — `REQUEST_CHANGES` for bugs/security, `APPROVE` for clean changes.
...
3. Deduplicate against existing unresolved review threads (see Step 6.4).
4. Submit the review per Step 8 — `REQUEST_CHANGES` for bugs/security/type safety, `APPROVE` for clean changes or minor nits only.

### 5d. Jules-involved PR review

**When `IS_JULES=true`**, this PR involves Jules (Google's coding agent, `google-labs-jules[bot]`). Jules only acts on feedback that explicitly mentions `@jules`. Your review MUST address Jules directly so it can pick up and act on your feedback.

1. Read the PR description and diff. Summarize the change in one paragraph.
2. If `JULES_CONTEXT` is `jules-review-submitted` or `jules-review-comment`, also fetch and read Jules' review/comment:

```bash
gh api /repos/$REPO_SLUG/pulls/$ARGUMENTS/reviews --jq '.[] | select(.user.login | test("jules")) | {state: .state, body: .body}'
gh api /repos/$REPO_SLUG/pulls/$ARGUMENTS/comments --jq '.[] | select(.user.login | test("jules")) | {path: .path, body: .body, line: .line}'
```

Address Jules' suggestions directly — if Jules proposed a change, evaluate whether it's correct and respond with your assessment.

3. Review for: bugs, type safety, security, AGENTS.md conventions (imports, Prisma, Redis, Zod, error handling, null semantics), missing tests, hardcoded values.
4. Deduplicate against existing unresolved review threads (see Step 6.4).
5. Submit the review per Step 8 — `REQUEST_CHANGES` for bugs/security/type safety, `APPROVE` for minor nits only.

**Critical: The review body MUST start with `@jules` on the first line.** This is required so Jules detects the review and can act on it. Format:
...
- You MUST perform the dedup check in Step 1 before any other action. When all prior findings are addressed, resolve all unresolved threads and submit APPROVE (Step 1b) — do NOT just print "Skipped" and stop.
- When `IS_JULES=true`, the review body or comment MUST start with `@jules` on the first line so Jules detects the feedback and can act on it.
