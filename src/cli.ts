...
async function cmdSync(
  ...
  if (result.conflicts.length > 0) {
    ...
    console.log();
    console.log(
      paint('yellow', '  First source was used. Review and manually reconcile if needed.'),
    );
    console.log();
  }
...
async function cmdInit(config: SkillSyncConfig, configPath?: string): Promise<void> {
  const path = configPath ? configPath : join(homedir(), '.config', 'skill-sync', 'config.json');
